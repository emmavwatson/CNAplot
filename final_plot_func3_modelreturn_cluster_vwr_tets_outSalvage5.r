#requires AneuFinder
#requires remove_nonconsecutive_outliers2
...
#bedfile <- file.path(x)
#model <- final_plot_func3_modelreturn_cluster_vwr_tets_outSalvage5(bedfile, 3e6, 0.7, blacklist_strict3, BSgenome.Hsapiens.1000genomes.hs37d5, "hg19", vwr)
#return(model)})
...
final_plot_func3_modelreturn_cluster_vwr_tets_outSalvage5 <- function(x,y,t,f,BS,assembly, vwr) {
    ...
    modelgr <- findCNVs(gr, method = "HMM", most.frequent.state = '4-somy', max.iter = 80)
    return(modelgr) }
